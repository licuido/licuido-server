...
// Response Schema of Order Status Master
const orderStatusResponse: JSONSchema = Schema.object()
  .prop(
    "page",
...
const orderStatusParams = Schema.object()
  .prop("offset", Schema.number())
  .prop("limit", Schema.number())
  .prop("search", Schema.string());

export const GET_ALL_ORDER_STATUS = {
  description: "Retireves all order status.",
  tags: ["Master Order Status"],
  response: makeResponseSchema(orderStatusResponse),
  query: orderStatusParams,
};
